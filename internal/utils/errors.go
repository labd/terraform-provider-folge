...
func CheckGetError(name string, id any, response ApiResponse, err error) *diag.ErrorDiagnostic {
	if err != nil {
		d := diag.NewErrorDiagnostic(
			fmt.Sprintf("Error retrieving %s with id %v", name, id),
			fmt.Sprintf("Could not retrieve %s with id %v, unexpected error: %s", name, id, err.Error()))
		return &d
	}
	...
	if response.StatusCode() != http.StatusOK {
		d := diag.NewErrorDiagnostic(
			fmt.Sprintf("Error retrieving %s with id %v", name, id),
			fmt.Sprintf("Could not retrieve %s with id %v, status code: %d (%s)",
